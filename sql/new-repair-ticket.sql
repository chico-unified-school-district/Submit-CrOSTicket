SELECT
  ...
  CASE WHEN
   (
    ( ([DRA].[CD] IS NULL) OR ([DRA].[CD] = 'W') )
    AND
    ( ([DRA].[CC] IS NULL) OR ([DRA].[CC] = 'NO Condition') )
   )
    THEN 'Closed'
...
WHERE
 ...
 (DRA.CC IN ( 'E', 'K','L','N','O','P','Q','S','T','U', 'V', 'X', 'Y' ) or DRA.CD in ('D','M', 'S'))
 AND DRA.DD IS NULL
